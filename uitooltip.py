...
class ToolTip(ui.ThinBoard):
	...
	NEGATIVE_COLOR = grp.GenerateColor(0.75, 0.3, 0.3, 1.0)
	POSITIVE_COLOR = grp.GenerateColor(0.0, 0.71, 0.84, 1.0)
	SPECIAL_POSITIVE_COLOR = grp.GenerateColor(0.0, 1.0, 0.0, 1.0) #bonus 1-4
	SPECIAL_POSITIVE_COLOR2 = grp.GenerateColor(1.0, 1.0, 0.0, 1.0) #bonus 6-7
	SPECIAL_POSITIVE_COLOR5 = grp.GenerateColor(0.0, 1.0, 0.0, 1.0) #bonus 5
...
class ItemToolTip(ToolTip):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0, window_type = player.INVENTORY, slotIndex = -1):
		...
		## item.ITEM_TYPE_BLEND
			...
			## Energy Crystal (51002) - special handling (no socket values)
			if self.itemVnum == 51002:
				self.AppendSpace(5)
				self.AppendTextLine("Klikni pro nahodny bonus", self.SPECIAL_POSITIVE_COLOR)
				self.AppendTextLine("Trvani: 1-3 hodiny", self.NORMAL_COLOR)
			elif metinSlot:
