...
class EnergyBar(ui.ScriptWindow):
	...
	def __init__(self):
		...
		# Energy Crystal affect tracking
	...
	def RemoveEnergyAffect(self):
		"""Called when AFFECT_ENERGY_CRYSTAL is removed"""
		self.isEnergyAffectActive = False
		self.energyAffectEndTime = 0
		self.energyAffectDuration = 0
		self.energyBonusType = 0
	...
	def RefreshStatus(self):
		# Use Energy Crystal affect system
		if self.isEnergyAffectActive:
			leftTime = max(0, self.energyAffectEndTime - app.GetGlobalTimeStamp())
			maxTime = self.energyAffectDuration
		else:
			leftTime = 0
			maxTime = 7200

		self.SetEnergy(leftTime, leftTime, maxTime)
	...
	def SetEnergy (self, point, leftTime, maxTime):
		...
		elif maxTime > 0 and ((leftTime * 100) / maxTime) < 15:
